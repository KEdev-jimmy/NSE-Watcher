...
function normalizeItem(item, forcedCategory) {
  ...
  return {
    id: normalizedId(item), title, summary, body, source, publishedAt, category,
    symbol, companyName, imageUrl, url, dividendAmount, exDate, paymentDate
  };
}
...
function dedupe(items) {
  ...
  return items.filter(item => {
    const key = item.id || `${item.title}|${item.publishedAt}`;
...
async function loadFeed() {
  ...
  if (intelResult.status === 'fulfilled') {
    intel = arrayFrom(intelResult.value, ['articles', 'items']).map(item => normalizeItem(item)).filter(Boolean);
  } else errors.push(`market-intel: ${intelResult.reason?.message || 'request failed'}`);
  if (dividendResult.status === 'fulfilled') {
    dividends = arrayFrom(dividendResult.value, ['declarations', 'dividends', 'items']).map(item => normalizeItem(item, 'Dividends')).filter(Boolean);
  } else errors.push(`dividends: ${dividendResult.reason?.message || 'request failed'}`);

  const items = sortNewest(dedupe([...intel, ...dividends].filter(withinWindow)));
  if (items.length) await enrichMissingCompanyMetadata(items);
  return { items, providerErrors: errors, partial: errors.length > 0 };
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === 'detail') {
      ...
      const item = normalizeItem(raw, categoryOf(raw));
      return json(res, 200, { source: 'MyStocks Africa', delayMinutes: 15, item });
    }

    if (action === 'company') {
      const rawSymbol = String(req.query.symbol || '').trim();
      if (!rawSymbol) return json(res, 400, { error: 'symbol is required' });
      const symbol = normalizeSymbol(rawSymbol) + '.KE';
      const data = await mystocks(`/companies/${encodeURIComponent(symbol)}/news?limit=50`);
      const rawItems = arrayFrom(data, ['items', 'articles', 'news']);
      const items = sortNewest(dedupe(rawItems.map(item => normalizeItem(item)).filter(Boolean).filter(withinWindow)));
      return json(res, 200, {
        source: 'MyStocks Africa',
        delayMinutes: 15,
        historyDays: HISTORY_DAYS,
