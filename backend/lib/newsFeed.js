...
function dateOf(item) {
  return String(first(item, ['publishedAt', 'published_at', 'date', 'createdAt', 'created_at', 'exDate', 'ex_date']) || '').trim();
}

function normalizeSymbol(value) { return String(value || '').trim().toUpperCase().replace(/\.KE$/, ''); }
...
function resolveFromTitle(item) {
  ...
  const match = NSE_COMPANIES.find(([symbol, company]) => title.includes(company.toLowerCase()) || title.includes(symbol.toLowerCase()));
  return match ? { symbol: match[0], companyName: match[1] } : { symbol: '', companyName: '' };
}

function categoryOf(item, fallback = 'Market') {
  const type = String(first(item, ['type', 'itemType', 'actionType', 'category']) || '').toLowerCase();
  if (type.includes('dividend')) return 'Dividends';
  if (type.includes('corporate') || type.includes('action') || type.includes('rights') || type.includes('bonus') || type.includes('split')) return 'Corporate Actions';
  if (type.includes('analysis') || type.includes('earnings') || type.includes('outlook')) return 'Analysis';
  if (type.includes('company') || type.includes('stock') || type.includes('news')) return 'Company News';
  return fallback;
}
...
function normalizeExternalNews(items) {
  return items.filter(item => item?.title && item?.url).filter(isRelevantExternalNews).map(item => normalizeItem(item, categoryOf(item, 'Market'))).filter(Boolean).filter(withinWindow);
}
...
async function enrichMissingCompanyMetadata(items) {
  const candidates = items.filter(item => !item.symbol || !item.companyName).slice(0, 30);
  for (const item of candidates) {
    try {
      ...
      if (item.url && item.verification === 'provider-only') item.verification = 'provider-linked';
    } catch (_) {}
  }
  return items;
}
...
async function loadMyStocksFeed() {
  const errors = [];
  const [intelResult, dividendResult] = await Promise.allSettled([mystocks('/market-intel?exchange=NSE&limit=50'), mystocks('/dividends/calendar?all=true')]);
  let intel = [];
  let dividends = [];
  if (intelResult.status === 'fulfilled') intel = arrayFrom(intelResult.value, ['articles', 'items']).map(item => normalizeItem(item)).filter(Boolean);
  else errors.push(`MyStocks market-intel: ${intelResult.reason?.message || 'request failed'}`);
  if (dividendResult.status === 'fulfilled') dividends = arrayFrom(dividendResult.value, ['declarations', 'dividends', 'items']).map(item => normalizeItem(item, 'Dividends')).filter(Boolean);
  else errors.push(`MyStocks dividends: ${dividendResult.reason?.message || 'request failed'}`);
  return { items: [...intel, ...dividends].filter(withinWindow), errors };
}

async function loadFeed() {
  const [myStocksResult, sourceResult] = await Promise.all([
    loadMyStocksFeed().catch(error => ({ items: [], errors: [`MyStocks: ${error?.message || 'request failed'}`] })),
    loadRssSources(),
  ]);
...
async function loadCompany(rawSymbol) {
  ...
  return sortNewest(dedupe(arrayFrom(data, ['items', 'articles', 'news']).map(item => normalizeItem(item)).filter(Boolean).filter(withinWindow)));
}
...
async function handle(req, res) {
  ...
  try {
    ...
    if (action === 'detail') {
      ...
      return json(res, 200, { source: 'MyStocks Africa', item: await loadDetail(id) });
    }
    if (action === 'company') {
      const rawSymbol = String(req.query.symbol || '').trim();
      if (!rawSymbol) return json(res, 400, { error: 'symbol is required' });
      return json(res, 200, { source: 'MyStocks Africa', historyDays: HISTORY_DAYS, symbol: normalizeSymbol(rawSymbol), items: await loadCompany(rawSymbol) });
    }
    return json(res, 400, { error: 'Unknown action', supportedActions: ['feed', 'detail', 'company'] });
  } catch (error) {
    return json(res, error.status || 502, { error: 'News data unavailable', detail: error.message, source: error.data ? 'MyStocks Africa' : 'NSE Watcher news aggregation', provider: error.data || null, items: [] });
  }
}
