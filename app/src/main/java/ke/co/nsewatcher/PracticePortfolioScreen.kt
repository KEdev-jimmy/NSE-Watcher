...
internal fun PracticePortfolioScreen(quoteFeed: List<Stock>, catalog: List<Stock>, initialMarket: MyStocksCache.MarketStatus,
    ...
    LaunchedEffect(Unit) {
        ...
        if (catalog.isEmpty()) { val rows = MarketData.loadCompanies(); if (rows.isNotEmpty()) onCatalog(rows) }
    }
    ...
    LaunchedEffect(resumed) {
        ...
        while (isActive) {
            market = MarketData.loadMarketStatus(); statusChecked = System.currentTimeMillis()
            if (MarketRefreshController.shouldRefreshQuotes(quoteFeed.isNotEmpty())) {
                val quotes = MarketData.loadStocks(); if (quotes.isNotEmpty()) onQuotes(quotes)
            }
            delay(30_000L)
        }
    }
