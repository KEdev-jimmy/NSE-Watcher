package ke.co.nsewatcher.data

import ke.co.nsewatcher.MarketRefreshController
import kotlinx.coroutines.CancellationException
import kotlinx.coroutines.CompletableDeferred
import kotlinx.coroutines.sync.Mutex
import kotlinx.coroutines.sync.withLock
import java.util.Locale

/**
 * Shared in-memory repository for daily technical-analysis history.
 *
 * Reopening Company Intelligence within the provider's refresh cadence reuses
 * the same verified observation. Manual refresh bypasses the cache. Empty or
 * unusable results are never retained for the full cache window.
 */
internal class TechnicalHistoryRepository(
    private val loader: suspend (String, Int) -> MyStocksCache.HistoryResult,
    private val nowMs: () -> Long = System::currentTimeMillis,
    private val maxAgeMs: Long = MarketRefreshController.REFRESH_INTERVAL_MS,
    private val maxEntries: Int = 64
) {
    private data class Key(val symbol: String, val lookbackDays: Int)
    private data class Entry(val result: MyStocksCache.HistoryResult, val loadedAtMs: Long)

    private val mutex = Mutex()
    private val entries = mutableMapOf<Key, Entry>()
    private val inFlight = mutableMapOf<Key, CompletableDeferred<MyStocksCache.HistoryResult>>()

    suspend fun load(
        symbol: String,
        lookbackDays: Int = 400,
        forceRefresh: Boolean = false
    ): MyStocksCache.HistoryResult {
        val key = Key(
            symbol = normalizeSymbol(symbol),
            lookbackDays = lookbackDays.coerceIn(260, 730)
        )
        if (key.symbol.isBlank()) return MyStocksCache.HistoryResult()

        var owner = false
        val pending = mutex.withLock {
            val cached = entries[key]
            if (!forceRefresh && cached != null && nowMs() - cached.loadedAtMs < maxAgeMs) {
                return cached.result
            }

            inFlight[key] ?: CompletableDeferred<MyStocksCache.HistoryResult>().also {
                inFlight[key] = it
                owner = true
            }
        }

        if (owner) {
            try {
                val result = loader(key.symbol, key.lookbackDays)
                mutex.withLock {
                    if (usable(result)) {
                        entries[key] = Entry(result, nowMs())
                        while (entries.size > maxEntries) {
                            val oldest = entries.minByOrNull { it.value.loadedAtMs }?.key ?: break
                            entries.remove(oldest)
                        }
                    }
                    inFlight.remove(key)
                }
                pending.complete(result)
            } catch (cancelled: CancellationException) {
                mutex.withLock { inFlight.remove(key) }
                pending.completeExceptionally(cancelled)
                throw cancelled
            } catch (_: Exception) {
                val empty = MyStocksCache.HistoryResult()
                mutex.withLock { inFlight.remove(key) }
                pending.complete(empty)
            }
        }

        return pending.await()
    }

    suspend fun clear(symbol: String? = null) {
        mutex.withLock {
            if (symbol == null) {
                entries.clear()
            } else {
                val normalized = normalizeSymbol(symbol)
                entries.keys.removeAll { it.symbol == normalized }
            }
        }
    }

    private fun normalizeSymbol(value: String): String =
        value.trim().uppercase(Locale.US).removeSuffix(".KE")

    private fun usable(result: MyStocksCache.HistoryResult): Boolean =
        result.points.isNotEmpty() &&
            result.interval.equals("1d", ignoreCase = true)
}

internal object TechnicalHistoryCache {
    private val repository = TechnicalHistoryRepository(
        loader = MarketData::technicalHistory
    )

    suspend fun load(
        symbol: String,
        lookbackDays: Int = 400,
        forceRefresh: Boolean = false
    ): MyStocksCache.HistoryResult =
        repository.load(symbol, lookbackDays, forceRefresh)

    suspend fun clear(symbol: String? = null) = repository.clear(symbol)
}
