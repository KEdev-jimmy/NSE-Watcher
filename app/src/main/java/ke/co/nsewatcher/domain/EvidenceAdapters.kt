...
/**
 * Adapters from provider-specific intelligence models into the neutral evidence contract.
 * These adapters do not fetch data, calculate new signals, or replace existing models.
 */
object EvidenceAdapters {
    fun fromNews(item: NewsItem): EvidenceRecord? {
        ...
        return EvidenceRecord(
            id = "news:${item.id}",
            symbol = item.symbol.ifBlank { null },
            companyName = item.companyName.ifBlank { null },
            type = when {
            ...
            claim = item.title,
            value = item.summary.ifBlank { null },
            source = item.source,
            sourceUrl = item.url.ifBlank { null },
            publishedAt = item.publishedAt.ifBlank { null }
        )
    }
    ...
    fun fromCompanyEvidence(evidence: CompanyIntelligenceCache.Evidence): EvidenceRecord? {
        ...
        return EvidenceRecord(
            ...
            symbol = evidence.symbol.ifBlank { null },
            type = evidenceTypeForCompanyClaim(evidence.claim),
            claim = evidence.claim,
            value = evidence.value.ifBlank { null },
            source = evidence.source,
            sourceUrl = evidence.url.ifBlank { null },
            observedAt = evidence.fetchedAt.ifBlank { null }
        )
    }

    fun fromMovementEvidence(
        symbol: String,
        ...
        return EvidenceRecord(
            ...
            value = evidence.description.ifBlank { null },
            source = evidence.source,
            sourceUrl = evidence.sourceUrl.ifBlank { null },
            publishedAt = evidence.date.ifBlank { null }
        )
    }
    ...
    private fun evidenceTypeForCompanyClaim(claim: String): EvidenceType {
        ...
        return when {
            ...
            normalized.contains("revenue") || normalized.contains("profit") || normalized.contains("eps") ||
                normalized.contains("roe") || normalized.contains("margin") || normalized.contains("financial") -> EvidenceType.FINANCIAL_RESULT
