...
fun WhyStockMovingSection(symbol: String) {
    ...
    Text(
        "Evidence around the latest price movement",
        color = MovementMuted,
        fontSize = 9.sp
    )
        ...
        Column(Modifier.padding(14.dp)) {
            when {
                loading -> Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Text(
                        "Checking price movement and dated company evidence…",
                        color = MovementMuted,
                        fontSize = 10.sp
                    )
                }
                result.error != null -> Text(result.error.orEmpty(), color = MovementMuted, fontSize = 10.sp)
                    ...
                    if (move == null) {
                        Text(
                            "A reliable movement window is not available yet.",
                            color = MovementMuted,
                            fontSize = 10.sp
                        )
                    } else {
                        Text(
                            move.change.ifBlank { "Movement available" },
                            color = if (move.change.startsWith("-")) Color(0xFFE04444) else MovementGreen,
                            fontSize = 24.sp,
                            fontWeight = FontWeight.ExtraBold
                        )
                        Text(
                        ...
                        if (evidence.isEmpty()) {
                            ...
                            evidence.forEachIndexed { index, item ->
                                ...
                                if (index < evidence.lastIndex) {
                                    HorizontalDivider(color = MovementBorder)
                                }
                            }
                        }
...
private fun MovementEvidenceRow(evidence: MovementIntelligenceCache.Evidence) {
    ...
    Row(
        ...
        Surface(Modifier.size(32.dp), RoundedCornerShape(9.dp), MovementLight) {
            Icon(
                if (evidence.eventType == "financial-results" || evidence.eventType == "dividend") {
        ...
        Column(Modifier.weight(1f)) {
            ...
            Text(
                listOf(evidence.date, evidence.source)
...
private fun movementWindow(move: MovementIntelligenceCache.Move): String {
    ...
    return if (move.from.isNotBlank() && move.to.isNotBlank()) {
        "$label • ${move.from} to ${move.to}"
    } else {
        label
    }
}
